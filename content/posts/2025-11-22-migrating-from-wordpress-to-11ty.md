...
- Regular blog posts
- Custom post types: [Projects](/projects) and [Experiments](/experiments)
...
The same [sitemap](/sitemap) structure that I had in WordPress was created with a Nunjuck template in 11ty. For example, here's a snippet from it:
...
- There's also a [sitemap.xml](/sitemap.xml) generator
- And a [RSS feed](/feed.xml) generator
...
For example, this is the frontmatter for one of [my old experiment](/experiments/3d-art/154-blender-geometry-nodes/) posts. You can see how taxonomies and meta fields are used (including `processImage` and `links`, these were all automatically converted from the WordPress post):
    ...
    - name: Sync files to VPS using rsync
      run: |
        rsync -avz --delete --no-owner --no-group \
          -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ${{ secrets.VPS_SSH_KEY }}" \
          _site/ ${{ secrets.VPS_USER }}@${{ secrets.VPS_HOST }}:/var/www/alfredbaudisch.com/_site/
```
