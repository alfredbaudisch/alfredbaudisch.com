...
updated: "2022-07-29T10:24:36.000Z"
tags: ["godot", "godot engine"]
categories: ["Godot Engine"]
featuredImage: "/media/wp-content/2022/07/GodotBroken3D-IssuesAndConfusion.jpg"
featuredImageThumb: "/media/wp-content/2022/07/GodotBroken3D-IssuesAndConfusion-768x432.jpg"
featuredImageSmall: "/media/wp-content/2022/07/GodotBroken3D-IssuesAndConfusion-300x169.jpg"
type: "post"
---
...
⚠️ **_UPDATE 1 (23/07/22): Seven of the issues listed here are gone or partially gone in Godot 4 (using Godot 4 alpha 12)_**_, of which one of them was fixed by myself, by updating Godot's code._

## By switching Scene tabs, you lose the Undo tree from the previous tab (2D/3D)

_UPDATE 1: Issue not present in Godot 4._

* * *

In a situation, I made changes to the material of a Mesh in the scene SuvSingleScene:

[![](/media/wp-content/2022/07/image-2-1024x665.png)](/media/wp-content/2022/07/image-2.png)
...
![](/media/wp-content/2022/07/image-3.png)
...
-   _**UPDATE 1: Partially solved with Godot 4.** Godot 4 has an "Advanced Mesh Import" dialog where you can choose the material overrides in a single sport._ _But **there are still 3 material assignment spots** per Mesh, which is still confusing and unclear._

<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/Godot4ImportDialog.jpg" alt="" loading="lazy">
  <figcaption>Godot 4 Material Override during import</figcaption>
</figure>

* * *

-   In Godot we have three different material assignments and/or override spots:
    1.  FIRST: Import a Mesh. In the Mesh Instance, **the Mesh child Node has a "Surface" property for each material of the Mesh**.
    2.  SECOND: In the **Mesh Instance itself, you also have spots for materials**.
    3.  THIRD: Then the **Mesh Instance also has a GeometryInstance resource where you can also set a MaterialOverride**, but it has only one spot, not one for each surface.
...
{% imageGallery [ { src: "/media/wp-content/2022/07/image-4.png", alt: "", caption: "", link: "/media/wp-content/2022/07/image-4.png" }, { src: "/media/wp-content/2022/07/image-5.png", alt: "", caption: "", link: "/media/wp-content/2022/07/image-5.png" } ] %}
<figure>
<video controls="" src="/media/wp-content/2022/07/GodotMaterialsFight.mp4"></video>
<figcaption>Materials fighting during runtime.</figcaption>
</figure>
...
[![](/media/wp-content/2022/07/image-6-1024x657.png)](/media/wp-content/2022/07/image-6.png)

Unfortunately, when running the test screen, at runtime, only the mesh's default materials were shown.

[![](/media/wp-content/2022/07/image-7-1024x635.png)](/media/wp-content/2022/07/image-7.png)
...
[![](/media/wp-content/2022/07/image-8.png)](/media/wp-content/2022/07/image-8.png)
...
<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/image-15.png" alt="" loading="lazy">
  <figcaption>UE material assignment</figcaption>
</figure>

## By default, Mesh re-Import resets all of your changes and overrides made in Godot

_UPDATE 1: Godot 4 seems to have this fixed. I didn't face it anymore. See update on the previous item._
...
Even by pulling my saved work from Git in order to try to recover my changes, they were still reset, because pulling from the repository is the same as starting fresh with a new `.import` directory, this way mesh and materials overrides are still lost.

Also by default I get cyclic resource errors when I import and re-import from scratch:

[![](/media/wp-content/2022/07/image-9.png)](/media/wp-content/2022/07/image-9.png)
...
[![](/media/wp-content/2022/07/image-10.png)](/media/wp-content/2022/07/image-10.png)
...
_**UPDATE 1:** In a similar scene from my original post with Godot 3, Godot 4 reports 0% to 4% GPU usage in the Viewport and less than 20% during runtime._

<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/Godor4-GPUUsage.jpg" alt="" loading="lazy">
  <figcaption>Godot 4 Viewport</figcaption>
</figure>

<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/Godot4-GPUUsageRuntime.jpg" alt="" loading="lazy">
  <figcaption>Godot 4 Runtime</figcaption>
</figure>
...
_UPDATE 1: Persists with Godot 4. I think this is by design, but it's not an ideal design choice. If I select a Node while in the Script editor, it shouldn't jump to the 2D/3D Viewport, it should stay in the Script editor in order for me to inspect the Node, get references, etc._

_UPDATE 2: I fixed it myself by adding a new Editor toggeable flag into Godot's source code. I [submitted the fix as a Pull Request](https://github.com/godotengine/godot/pull/63344). If it's not merged officially, I'll end up always compiling Godot from source to have this feature active for my work with Godot._

_**UPDATE 3: ⚠️ The Feature "Don't switch to 2D/3D viewports when selecting nodes while in Script Editor" has been merged and is now available in Godot, starting from [Godot 4.0-alpha13](https://godotengine.org/article/dev-snapshot-godot-4-0-alpha-13) and [Godot 3.5RC8](https://godotengine.org/article/release-candidate-godot-3-5-rc-8).**_

<figure>
<video controls="" src="/media/wp-content/2022/07/GodotSweitchSolved.mp4"></video>
<figcaption>My fix in action, no more viewport jumping, yay! As a matter of factor, I learned that "Viewports" are called "EditorPlugins" in Godot's source code.</figcaption>
</figure>
...
<figure>
<video controls="" src="/media/wp-content/2022/07/GodotSwitchjingAnnoying.mp4"></video>
<figcaption>I bet you will also get triggered by watching this.</figcaption>
</figure>
...
<video controls="" src="/media/wp-content/2022/07/GodotEditorViewportFighting.mp4"></video>

## Confusing and Conflicting Undo and Duplicate behaviors between the Script Editor and the Scene Tree (2D/3D)

-   _UPDATE 1: Partially solved with Godot 4. When selecting a Node in the Scene Tree, and pressing CTRL+D, the Node gets correctly duplicated, instead of a line of code. Also, CTRL+Z correctly undoes the highlighted widget (Scene Tree or Script)._
...
<video controls="" src="/media/wp-content/2022/07/GodotDuplicateAndUndo.mp4"></video>
...
[![](/media/wp-content/2022/07/image-11.png)](/media/wp-content/2022/07/image-11.png)

## Inconsistent Autoplay and Looping animation behaviour (2D/3D)

-   _UPDATE 1: I found [bug reports](https://github.com/godotengine/godot/issues/34394) for this, so issue confirmed._
-   _**UPDATE 2: With Godot 4′ Import Dialog, when you mark an animation as "Loop Mode: Linear", it loops correctly.** But checking this in the Animation Player still does not work, but that's ok, it's just a bug and Godot 4 is still in Alpha._

<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/image-23.png" alt="" loading="lazy">
  <figcaption>Godot 4 Import Dialog animation options</figcaption>
</figure>
...
-   The only way to make it consistent is to force playing the animation with code, in a `_ready` override.
-   I don't even have a way to properly report or duplicate this one, because it's random, regardless of the animation type being played.
...
<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/image-12.png" alt="" loading="lazy">
  <figcaption>Autoplay on Load</figcaption>
</figure>
<figure class="wp-block-image">
  <img src="/media/wp-content/2022/07/image-14.png" alt="" loading="lazy">
  <figcaption>Animation Looping</figcaption>
</figure>
<figure>
<video controls="" src="/media/wp-content/2022/07/Godot4LoopsOnlyViewport-NoTitle.mp4"></video>
<figcaption>In Godot 4, Animation Looping works in the viewport, not at runtime (could be a bug, since it's in Alpha).</figcaption>
</figure>

## Lack of Camera and Position input data in Shaders

_UPDATE 1: [A proposal by @patrick\_exe](https://twitter.com/patrick_exe/status/1550945716478119938) has been created to address this._
...
![](/media/wp-content/2022/07/image-13.png)

In Godot, we have available some matrices such as: MODEL\_MATRIX, INV\_VIEW\_MATRIX, MODELVIEW\_MATRIX and then we have to perform matrix transformations to get the intended input data (for example, to get Absolute World Position or Camera Position in World Space).
